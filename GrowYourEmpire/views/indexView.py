...
from GrowYourEmpire.models import Test,Question, Option,Village,Student,New
...
def test(request,testN):
	if not request.user.is_authenticated:
		...
		return render(request,'GrowYourEmpire/test.html',{'questions':questions,'questionOpts':questionOpts})
...
def news(request):
	if not request.user.is_authenticated:
		...
		news = New.objects.all().order_by("-registeredDateTime")
		return render(request,'GrowYourEmpire/news.html',{'news':news})
